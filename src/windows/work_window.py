...
class WorkWindow:
    def __init__(self, master, project):
        ...
        self.MENUBAR_WIDTH = self.MenuBar.winfo_width()
        self.MENUBAR_HEIGHT = self.MenuBar.winfo_height()
        self.MENUBAR_x = 0
        self.MENUBAR_y = 0
        ...
        self.mainCanves = MainCanvas(self.window, width=self.MAIN_CANVAS_WIDTH, height=self.MAIN_CANVAS_HEIGHT, x=self.MAIN_CANVAS_X, y=self.MAIN_CANVAS_Y, bg=WHITE)
        self.ModulesBar = ModulesBar(self.window, bg=LIGHTGRAY, width=self.MODULE_BAR_WIDTH, height=self.MODULE_BAR_HEIGHT, x=self.MODULE_BAR_X,y=self.MODULE_BAR_Y)

        self.window.protocol("WM_DELETE_WINDOW", lambda: master.destroy())

    def create_menu_bar(self):
        self.MenuBar = tk.Menu(self.window, bg=DARKGRAY)
